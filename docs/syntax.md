...
## 式
TypeScript(JavaScript)と同じような式が使えます。

### 変数

整数型と文字列型の単純変数（スカラ）および配列が利用可能です。変数には型があります。変数の宣言時に型を明示するか、初期化により間接的に型を指定する必要があります。
変数にはグローバル変数とローカル変数があります。ローカル変数は関数の中で宣言されたものです。グローバル変数は、関数の中からでも参照可能です。
...
制御構文として、JavaScriptの ifや、while、do whileが使用できます。
...
秀丸マクロの関数・サブルーチンは単なるラベルであり、call で呼び出すと関数・サブルーチンとして動きますが、call以外でこのラベルに至った場合サブルーチンの中身が実行されてしまいます。これを避けるために、gotoによりサブルーチンを迂回するコードを生成しています。
...
また、型情報の v は、戻り値をとらない(void)ことと、引数がないことを意味します。
...
var x = foo(n);
// ...
foo = function (x: number): number { return 1; }  // 関数fooの定義
