...
変数にはグローバル変数とローカル変数があります。ローカル変数は関数の中で宣言されたものです。グローバル変数は、どこからでも（関数の中からでも）参照可能です。ブロックスコープはサポートしていません。変数や関数が同じスコープで重複するとエラーになります。
...
### 整数リテラル
10進および16進数の整数値をサポートしています。16進数は0xあるいは0Xで始まる形式です。
